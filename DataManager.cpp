...
Resource::Resource() {
    ...
    Console::WriteLine("Создан объект класса Resource");
}

void Resource::ReadDataFile() {
    try {
...
Object^ Resource::MakeNode(DataType type, String^ line) {
    Object^ node = nullptr;
...
void Resource::AddNode(DataType type, Object^ node) {
    switch (type) {
    case type_Link:
...
void Resource::DeleteNode(DataType type, Object^ node) {
    Link^ t_link = nullptr;
    Book^ t_book = nullptr;
...
List<Object^>^ Resource::FindNode(DataType type, String^ line) {
    int find_count = 0;
